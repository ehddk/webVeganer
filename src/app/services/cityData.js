...
export default function CityData({onSelectLocation}) {
  ...
  if (!hangjungdong || !hangjungdong.sido || !Array.isArray(hangjungdong.sido)) {
  ...
  const handleSigugunClick = (sigugunCode,sigugunCodeNm) => {
    ...
    const selectedDongList = hangjungdong.dong
      .filter(item => item.sido === selectedSido && item.sigugun === sigugunCode)
      .map(item => item.codeNm);

    const selectedSigugunCodeNm = hangjungdong.sigugun.find(item => item.sido === selectedSido && item.sigugun === sigugunCode)?.codeNm;
 ...
 // console.log("click",sigugunList[0].codeNm)
  ...
  return (
    <div style={{ display: "flex" }}>
      <div style={{ borderRight: "3px solid navy", width: "200px", overflow: "auto", maxHeight: "450px" }}>
        <ul style={{ listStyleType: "none", fontSize: "20px", padding: "15px" }}>
          {codeNmList.map((codeNm, index) => (
            <li key={index} onClick={() => handleSidoClick(hangjungdong.sido[index].sido)} style={{
              background: selectedSido === hangjungdong.sido[index].sido ? "navy" : "transparent",
              color: selectedSido === hangjungdong.sido[index].sido ? "white" : "black",
              cursor: "pointer",
              padding: "10px",
            }}>
              {codeNm}
      ...
      <div style={{ overflowY: "auto", width: "150px", fontSize: "20px", padding: "14px 10px 0 0px", maxHeight: "430px", borderRight: "3px solid navy", marginLeft: "5px" }}>
        <ul style={{ listStyleType: "none" }}>
          {sigugunList.map((sigugun, index) => (
            <li key={index} onClick={() => handleSigugunClick(sigugun.sigugun)} style={{
              background: selectedSigugun === sigugun.sigugun ? "navy" : "transparent",
